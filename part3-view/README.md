...
Part 3 兩個群組都已完成，frame 覆蓋 **137/138**
（唯一未覆蓋的 `377:63552` 在 Figma 已被刪除，`tools/manifest_p3.json` 尚未同步）。

**② 預約管理視圖**

| 視圖 | 定稿 | 狀態 |
|---|---|---|
| 時間軸 | 3-1-1 + 3-1-2（9 frames） | ✅ |
| 空間圖 | 3-2（2 frames） | ✅ |
| 清單 | 3-3-1（8 桌機 + 8 手機）+ 3-3-2（3 frames） | ✅ |

**① 新增／修改預約**（108 frames）

本體是一個 350 寬的抽屜（與修改預約同槽位），不是新畫面。
2-1-x 階層項目為主線，2-2-x／2-3-x／2-4-x 是它的刪減版：

| 預約模式 | 預約項目 | 預約單位 | 時間提示 |
|---|---|---|---|
| 階層項目 hier | 兩層 | 有 | 請選擇時間 |
| 服務項目 service | 單層 | 有 | 選擇時間 |
| 基本人數 basic | 無 | 有 | 選擇時間 |
| 總量控管 capacity | 無 | 無 | 選擇時間 |

用 `?bmode=hier|service|basic|capacity` 切換。

> ⚠️ 這張表的差異**文案稽核抓不到**（「選擇時間」是「請選擇時間」的子字串），
> 是實測四種模式的定稿文字集合差異得出的，只能靠第 3 關的斷言守住。
...
| 區塊 | 手機版來源 |
|---|---|
| 清單卡片、狀態 dropdown、burger header、底部＋預約 | 定稿 `3-3-1 *_M`（390） |
| 時間軸、空間圖 | **本專案設計**（定稿沒畫手機稿） |

手機版的主要變化：左側主導航收成 burger（滑入 + 遮罩）、8 個狀態分頁收成 dropdown、
三視圖切換改滿版、＋預約改底部固定鈕、預約卡改直排（時間變灰底列、備註變灰底區塊、
meta 靠右並多一顆展開鈕）。

時間軸在手機改為**依時間排序的日視圖列表**——甘特格線在 390 寬無法等比呈現。
刻意重用清單的預約卡元件，而不是另外發明一套。
...
| 項目 | 決定 | 日期 |
|---|---|---|
| 服務時長的小時上限 | **23 小時**。定稿的可視窗格只看得到 0–7，實際上限從靜態圖看不出來 | 2026-08-10 Ian |

⚠️ 級距是**每 5 分**（定稿如此），所以可選的最大值是 **23小時55分**，不是 23:59。
要做到 23:59 得把分改成每 1 分，那會牴觸定稿級距，需要設計同意。
