...
const api = (() => {
  ...
  /* 桌次分組。⚠️ 時間軸左欄的容量是「1~2」（無括號、波浪號），
     空間圖桌位卡才是「(1-4)」——兩者是不同欄位不同格式，不要一起改。 */
  const GROUPS = [
  ...
  const BOOKINGS = [
    ...
    /* 終止／待處理狀態各補一筆：原本 mock 裡完全沒有這三種，
       清單的「待付款/綁卡」「未到店」「取消預約」永遠是 0，
       那三張定稿等於從來沒被有資料的狀態驗證過（走查 B 抓到）。 */
    B("b14", "F5", "14:00", 120, "楊", "0912345678", 2, 0, "payment"),
    B("b15", "O3", "15:00", 120, "邱", "0988877777", 2, 0, "noshow"),
