...
const WHEEL = {
  ...
  /* 服務時長與預約時間是同一元件套不同數值：分同樣每 5 分（不是每 15 分）、
     ...
     小時上限＝23：定稿的可視窗格只看得到 0–7，實際上限從靜態圖看不出來，
     2026-08-10 由 Ian 拍板「先定為 23 小時」。這是**產品決定**，不是從 Figma 推導的值。
     ⚠️ 級距是每 5 分（定稿如此），所以可選的最大值是 23小時55分而不是 23:59。
        若之後要做到 23:59，分必須改成每 1 分——那會牴觸定稿的級距，需要設計同意。 */
